...
semi_datasets_folder = "../datasets/semi_synthetic/"
...
for(i in 1:length(files)){
  data_file = files[i]
  cat("running CausalTree on semi-synthetic dataset: ", data_file, "\n")
  ...
  # fit and prune the causal tree
  d_train_noT = d_train[, !colnames(d_train) %in% c("treatment")]
  d_test_noT = d_test[, !colnames(d_test) %in% c("treatment")]
  
  tree_model = causalTree(Y~., data = d_train_noT, treatment = d_train$treatment,
  ...
  if((i %% 10 == 0)|(i == length(files))){
    write.csv(res_df,
              file = paste0("./NEW_semi_CausalTree_results_full.csv"),
              row.names = FALSE,
              quote = TRUE)
    write.csv(res_df_summary,
              file = paste0("./NEW_semi_CausalTree_results.csv"),
              row.names = FALSE,
              quote = TRUE)
  }
}
